...
function SellerName({ seller }: { seller: Principal }) {
  ...
  const displayName =
    profile?.displayName?.trim() || truncatePrincipal(seller.toString());
...
function ListingDetailModal({
  ...
  open,
  onClose,
}: { listing: Listing; open: boolean; onClose: () => void }) {
  ...
  const sellerName =
    sellerProfile?.displayName?.trim() || truncatePrincipal(sellerKey);
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <span
                className={`text-xs px-2 py-0.5 rounded-full border font-medium ${CONDITION_COLORS[listing.condition] ?? "badge-orange"}`}
              >
                {listing.condition}
              </span>
              <span
            ...
            <Button
              ...
              onClick={() =>
                markSold.mutate(listing.id, {
                  ...
                  onError: (err) =>
                    toast.error(
                      `Failed to mark sold: ${err instanceof Error ? err.message : "Unknown error"}`,
                    ),
                })
              }
            >
              ...
              >
                <div className="relative overflow-hidden">
                  ...
                  {listing.isSold && (
                    ...
                    >
                      <span className="text-white font-display text-xl font-black rotate-[-20deg]">
                        SOLD
                      </span>
                    </div>
                  )}
